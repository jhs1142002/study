...
echo # 로컬 보안 정책 덤프 #								>>[WIN]%COMPUTERNAME%.txt
secedit /export /cfg C:\secpol.txt 					>>[WIN]%COMPUTERNAME%.txt
echo # 개체 엑세스 감사 #									>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditObjectAccess"		>>[WIN]%COMPUTERNAME%.txt
echo # 계정 관리 감사 #									>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditAccountManage"	>>[WIN]%COMPUTERNAME%.txt
echo # 계정 로그온 이벤트 감사 #							>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditAccountLogon"		>>[WIN]%COMPUTERNAME%.txt
echo # 권한 사용 감사 #									>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditPrivilegeUse"		>>[WIN]%COMPUTERNAME%.txt
echo # 디렉토리 서비스 엑세스 감사 #							>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditDSAccess"			>>[WIN]%COMPUTERNAME%.txt
echo # 로그온 이벤트 감사 #								>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditLogonEvents"		>>[WIN]%COMPUTERNAME%.txt
echo # 시스템 이벤트 감사 #								>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditSystemEvents"		>>[WIN]%COMPUTERNAME%.txt
echo # 정책 변경 감사 #									>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditPolicyChange"		>>[WIN]%COMPUTERNAME%.txt
echo # 프로세스 추적 감사 #								>>[WIN]%COMPUTERNAME%.txt
type C:\secpol.txt |find /i "AuditProcessTracking"	>>[WIN]%COMPUTERNAME%.txt
echo.												>>[WIN]%COMPUTERNAME%.txt
echo.												>>[WIN]%COMPUTERNAME%.txt
echo [W-58 END]										>>[WIN]%COMPUTERNAME%.txt
echo  ============================================================================== >>[WIN]%COMPUTERNAME%.txt
echo [W-64 START]									>>[WIN]%COMPUTERNAME%.txt
echo [W-64 "화면 보호기 설정 "] 							>>[WIN]%COMPUTERNAME%.txt
echo # 화면보호기 설정 확인 #								>>[WIN]%COMPUTERNAME%.txt
reg query "HKCU\Control Panel\Desktop" /v "ScreenSaveActive" >>[WIN]%COMPUTERNAME%.txt
echo # 화면보호기 해제 암호 #								>>[WIN]%COMPUTERNAME%.txt
reg query "HKCU\Control Panel\Desktop" /v "ScreenSaverIsSecure" >>[WIN]%COMPUTERNAME%.txt
echo # 화면보호기 대기 시간 #								>>[WIN]%COMPUTERNAME%.txt
reg query "HKCU\Control Panel\Desktop" /v "ScreenSaveTimeOut">>[WIN]%COMPUTERNAME%.txt
echo.												>>[WIN]%COMPUTERNAME%.txt
echo.												>>[WIN]%COMPUTERNAME%.txt
echo [W-64 END]										>>[WIN]%COMPUTERNAME%.txt
echo  ============================================================================== >>[WIN]%COMPUTERNAME%.txt